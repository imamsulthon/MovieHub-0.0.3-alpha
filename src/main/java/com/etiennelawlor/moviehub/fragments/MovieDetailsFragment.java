...
public class MovieDetailsFragment extends BaseFragment {
    ...
    @BindView(R.id.votingAverage_tv)
    TextView votingAverage;
                        ...
                        public void onSuccess() {
                            ...
                            Palette.from(bitmap).generate(new Palette.PaletteAsyncListener() {
                                public void onGenerated(Palette palette) {
                                    ...
                                    setUpTitleTextColor(votingAverage, palette);
                ...
                .subscribe(new Action1<FullMovie>() {
                    ...
                    public void call(FullMovie fullMovie) {
                        if (fullMovie != null) {
                            ...
                            setUpRevenue();
                            setUpRating();
                            setUpDirector(movieCreditsEnvelope);
...
//                            errorLinearLayout.setVisibility(View.VISIBLE);
    ...
    private void setUpDirector(MovieCreditsEnvelope movieCreditsEnvelope) {
        List<String> directorName = new ArrayList<String>();
        if (movieCreditsEnvelope != null) {
            List<MovieCredit> crews = movieCreditsEnvelope.getCrew();
            if (crews != null && crews.size()>0) {
                MovieCredit movieCredit = new MovieCredit();
                for (int i = 0; i<crews.size(); i++) {
                    movieCredit = crews.get(i);
                    if (movieCredit.getJob().equals("Director")) {
                        directorName.add(crews.get(i).getName());
                    }
                }
            } else {
                directorName = null;
        ...
        if (directorName.size() > 0) {
            directorTextView.setText(TextUtils.join(", ", directorName));
        } else {
            directorTextView.setText("Director Not Found");
        }
    }
    ...
    private void setUpVoteAverage() {
        String voteValue = String.valueOf(movie.getVoteAverage());
        votingAverage.setText(voteValue + "/10");

    }

    private void setUpTagLine() {
    ...
    private void setUpOriginalTitle() {
